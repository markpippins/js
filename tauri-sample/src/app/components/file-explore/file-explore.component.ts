...
export class FileExploreComponent implements OnInit {
  ...
  constructor(@Inject(FileService) public fileService: FileService) {}

  async ngOnInit() {
    if (this.fileService.isTauri) {
      // set reasonable default for Tauri desktop
  ...
  async loadLocalFiles() {
    try {
      this.localFiles = await this.fileService.getLocalFiles(this.localPath);
  ...
  async loadRemoteFiles() {
    ...
    try {
      this.remoteFiles = await this.fileService.getRemoteFiles(
        this.remotePath, 
  ...
  async connect() {
    try {
      ...
      if (this.connectionConfig.protocol === 'sftp') {
        result = await this.fileService.connectSftp({
        ...
        result = await this.fileService.connectFtp({
          host: this.connectionConfig.host,
          port: this.connectionConfig.port,
  ...
  async disconnect() {
    try {
      await this.fileService.disconnect();
